...
_GROWTH_EVENT_SOURCES = ("chat", "calendar", "todo", "email", "system")
_GROWTH_PRIORITIES = ("low", "normal", "high")
_CHECKIN_RESULTS = ("done", "partial", "missed")
...
@app.command()
def growth(
    message: str = typer.Option(..., "--message", "-m", help="Message to send through the growth path"),
    event_type: str = typer.Option("user_message", "--event-type", help="Growth event type"),
    user_id: str = typer.Option("cli-user", "--user-id", help="Growth user ID"),
    session_id: str = typer.Option("cli:growth", "--session", "-s", help="Growth session ID"),
    source: str | None = typer.Option(None, "--source", help="Growth event source"),
    priority: str = typer.Option("normal", "--priority", help="Growth event priority"),
    status: str | None = typer.Option(None, "--status", help="Task status for task_update events"),
    result: str | None = typer.Option(None, "--result", help="Check-in result for checkin events"),
    mood: int | None = typer.Option(None, "--mood", min=1, max=5, help="Check-in mood score"),
    energy: int | None = typer.Option(
        None, "--energy", min=1, max=5, help="Check-in energy score"
    ),
    remember: list[str] | None = typer.Option(
        None, "--remember", help="Explicit long-term memory items to persist"
    ),
    evidence: list[str] | None = typer.Option(
        None, "--evidence", help="Workspace-relative file evidence for the current turn"
    ),
    evidence_text: list[str] | None = typer.Option(
        None, "--evidence-text", help="Inline evidence text for the current turn"
    ),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Config file path"),
    ...
    """Run the Hermes-style growth path directly."""
    ...
    validated_event_type = _validate_growth_choice("event type", event_type, _GROWTH_EVENT_TYPES)
    validated_priority = _validate_growth_choice("priority", priority, _GROWTH_PRIORITIES)
    validated_source = _resolve_growth_source(validated_event_type, source)
    payload = _build_growth_payload(
    ...
    async def run_once() -> str:
        runtime = NanobotRuntimeAdapter.from_config(
            config_path=config,
            workspace=runtime_config.workspace_path,
        )
        memory_service = HermesStyleMemoryService(runtime.workspace)
        orchestrator = HermesStyleGrowthOrchestrator(
            runtime,
            memory_service=memory_service,
        )
        event = GrowthEvent(
            id=f"growth-{uuid4().hex}",
        ...
        return await orchestrator.handle(event)

    response = asyncio.run(run_once())
    _print_agent_response(response, render_markdown=markdown)


def _validate_growth_choice(label: str, value: str, choices: tuple[str, ...]) -> str:
    if value in choices:
        return value
    joined = ", ".join(choices)
    console.print(f"[red]Error: invalid {label} '{value}'. Choose one of: {joined}[/red]")
    raise typer.Exit(1)


def _resolve_growth_source(event_type: str, source: str | None) -> str:
    if source is not None:
        return _validate_growth_choice("source", source, _GROWTH_EVENT_SOURCES)
    ...
    defaults = {
        ...
        "scheduled_wakeup": "system",
        "weekly_review": "system",
        "external_signal": "system",
    }
...
def _build_growth_payload(
    ...
    *,
    ...
    if evidence_text:
        payload["evidence_texts"] = evidence_text

    if event_type == "task_update" and status:
        payload["status"] = status

    if event_type == "checkin":
        payload["note"] = message
        if result is not None:
            payload["result"] = _validate_growth_choice("check-in result", result, _CHECKIN_RESULTS)
        if mood is not None:
            payload["mood"] = mood
