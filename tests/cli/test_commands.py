...
def test_growth_command_builds_event_and_calls_orchestrator(
    monkeypatch, tmp_path: Path
) -> None:
    config = Config()
    config.agents.defaults.workspace = str(tmp_path / "growth-workspace")
    seen: dict[str, object] = {}

    monkeypatch.setattr(
        "nanobot.cli.commands._load_runtime_config",
        lambda config_arg=None, workspace_arg=None: (
            seen.__setitem__("config_arg", config_arg),
            seen.__setitem__("workspace_arg", workspace_arg),
            config,
        )[-1],
    )
    monkeypatch.setattr(
        "nanobot.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("templates_workspace", path),
    )
    ...
    class _FakeRuntimeAdapter:
        ...
        def from_config(cls, config_path=None, *, workspace=None):
            seen["runtime_config_path"] = config_path
            seen["runtime_workspace"] = workspace
            return cls(workspace)

    class _FakeMemoryService:
        def __init__(self, workspace: Path) -> None:
            seen["memory_workspace"] = workspace

    class _FakeOrchestrator:
        def __init__(self, runtime, *, memory_service) -> None:
            seen["runtime"] = runtime
            seen["memory_service"] = memory_service
    ...
    monkeypatch.setattr(
        "nanobot.growth.integration.nanobot_runtime.NanobotRuntimeAdapter",
        _FakeRuntimeAdapter,
    )
    monkeypatch.setattr(
        "nanobot.growth.memory.snapshot.HermesStyleMemoryService",
        _FakeMemoryService,
    )
    monkeypatch.setattr(
        "nanobot.growth.orchestrator.service.HermesStyleGrowthOrchestrator",
        _FakeOrchestrator,
    )
    monkeypatch.setattr(
        "nanobot.cli.commands._print_agent_response",
        lambda content, *, render_markdown, metadata=None: seen.update(
            output=content,
    ...
    config_file = tmp_path / "growth-config.json"
    result = runner.invoke(
        app,
        [
            "growth",
    ...
    assert result.exit_code == 0
    assert seen["config_arg"] == str(config_file)
    assert seen["templates_workspace"] == config.workspace_path
    assert seen["runtime_config_path"] == str(config_file)
    assert seen["runtime_workspace"] == config.workspace_path
    assert seen["memory_workspace"] == config.workspace_path
    assert seen["output"] == "growth-ok"
    assert seen["render_markdown"] is False

    event = seen["event"]
    assert event.type == "user_message"
    assert event.user_id == "alice"
    assert event.session_id == "growth-session"
    assert event.payload == {"text": "Please answer directly."}
    assert event.source == "chat"


def test_growth_command_supports_task_update_payload(
    monkeypatch, tmp_path: Path
) -> None:
    config = Config()
    config.agents.defaults.workspace = str(tmp_path / "growth-workspace")
    ...
    class _FakeOrchestrator:
        def __init__(self, runtime, *, memory_service) -> None:
            del runtime, memory_service

        async def handle(self, event) -> str:
            seen["event"] = event
            return "growth-task-ok"
    ...
    result = runner.invoke(
        ...
        [
            "growth",
            "-m",
            "Task is blocked on external review.",
            "--event-type",
            "task_update",
            "--status",
            "blocked",
    ...
    assert event.source == "todo"
    assert event.priority == "high"
    assert event.payload == {
        "text": "Task is blocked on external review.",
        "status": "blocked",
        "remember": ["Avoid late deep work."],
    }


def test_growth_command_supports_checkin_payload(
    monkeypatch, tmp_path: Path
) -> None:
    config = Config()
    config.agents.defaults.workspace = str(tmp_path / "growth-workspace")
    ...
    result = runner.invoke(
        ...
        [
            "growth",
            "-m",
            "Completed half of the writing block.",
            "--event-type",
            "checkin",
            "--result",
            "partial",
    ...
    assert event.source == "chat"
    assert event.payload == {
        "text": "Completed half of the writing block.",
        "note": "Completed half of the writing block.",
        "result": "partial",
        "mood": 3,
        "energy": 2,
    }
...
def test_growth_command_supports_evidence_payload(
    monkeypatch, tmp_path: Path
) -> None:
    config = Config()
    config.agents.defaults.workspace = str(tmp_path / "growth-workspace")
    ...
    assert event.payload == {
        ...
        "evidence_texts": ["Inline note: watch late nights."],
    }


def test_growth_command_rejects_invalid_event_type(tmp_path: Path) -> None:
    result = runner.invoke(
        app,
        [
            "growth",
            ...
            "unknown_event",
            "--config",
            str(tmp_path / "growth-config.json"),
        ],
    )

    assert result.exit_code == 1
    assert "invalid event type" in result.stdout
